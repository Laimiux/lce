# com.laimiux.lce.UCE (Unit / Content / Error)
 `UCE` is an LCE type used to represent loading or content or custom error. It come from [Lce Library](https:/github.com/Laimiux/lce)
- See `.ai/docs/README.md` for other LCE types.

## Initialization
- Use `UCE.loading()` to create loading type
- Use `UCE.content(value)` to create content type
- Use `UCE.error(throwable)` to create an error type
